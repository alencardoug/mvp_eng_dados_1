...
| P4 | O candidato do pacote foi montado com o código que o B5 vai usar, e é o estado dos bancos quando o B5 começa | `make recovery-pack`, depois `make recovery-verify CONTRA_O_BANCO=1`; o manifesto registra o `commit`; e **nada roda entre o `recovery-pack` e o §7.1** — nenhuma sincronização, DAG ou geração —, porque o recuo da fase 1 (§7.4) devolve os bancos ao corte | **[medido]** um candidato de ensaio: corte `2026-09-24T23:43:04Z`, `commit` `561edc2`, conferido contra os bancos. **Refazer na véspera do B5 (D61)** — o primeiro passo depois da autorização (P6), no *checkout* antigo, com a ponta revisada —, e do corte até o §7.1 nada roda. Nada compara o `commit` na restauração — ele é a procedência do pacote, e a do candidato promovido no passo 9 deve ser a ponta que o B5 clonou |
| P5 | Uma cópia do candidato refeito (P4) fora do repositório, que sirva de `RECOVERY_DIR` | `mkdir ~/mvp_ed1-recovery-<corte> && cp -a data/recovery/candidato ~/mvp_ed1-recovery-<corte>/`; depois `cd ~/mvp_ed1-recovery-<corte>/candidato && sha256sum -c checksums.sha256` | **[planejado]** — depois do `make reset`, o pacote é a única cópia da memória do armazém (capturas, certificados, SCD, quarentena); a cópia custa ~31 MB. O diretório é um `RECOVERY_DIR` pronto — `export RECOVERY_DIR=~/mvp_ed1-recovery-<corte>` —, e é o recuo se o *checkout* antigo se perder (§7.4). **[medido]** numa cópia descartável, em 25/09/2026: os nove arquivos do `checksums.sha256` conferem, e o `make recovery-verify` com esse `RECOVERY_DIR` confere o candidato |
...
**A ordem não é a da tabela.** A P6, a autorização, vem antes da P4 e da P5: o candidato é refeito
e copiado na véspera, depois dela (D61), e a P7 é conferida linha a linha.
...
| 4 | `make airbyte-down`, depois `mv ~/.airbyte/abctl/data ~/.airbyte/abctl/data.<data>` e `mv airbyte/terraform.tfstate airbyte/terraform.tfstate.<data>` | o nó `airbyte-abctl-control-plane` fora; o diretório de dados **apartado, não apagado** — é a armadilha do `PG_VERSION` da Execução Local §6, com o remédio documentado —, e o estado do Terraform apartado junto: ele nomeia as conexões do Airbyte que acabou de sair (RVB5-2-01). O contador de *jobs* do Airbyte (43) sai junto, e é a D50 que o reconstrói no passo 9 |
...
|---|---|---|---|---|
| 1 | Base | `make medir ALVO=up` → `ALVO=migrate` → `ALVO=seed-data` → `ALVO=seed-legacy` | bancos | a rede `mvp_ed1_default` criada pela garantia (D55, D57); `make migrate-status` e `make migrate-legacy-status` → `(head)` nas duas origens, e `No new upgrade operations detected.` no legado; a cobertura, conferida no `check` da linha 4 (`test_cobertura`, o manifesto do legado novo) |
...
| 4 | Transformação | `make airbyte-up` (pausa o *streaming*) → `make medir ALVO=dbt-build` → `make medir ALVO=check` | + Airbyte | o primeiro *build* completo: `PASS=`, `caminhos_de_ingestao_reconciliam`, os 14 testes da tag `fronteira` (`dbt ls --select tag:fronteira --resource-type test`); `N passed`, com cada pulado e o motivo |
...
|---|---|---|---|
| 1 | §7.1, linhas 1–4, antes do `make reset` | os três bancos antigos, intactos; o candidato; a cópia da P5 | a memória não se perdeu, mas o que o desmonte tirou não volta só subindo os serviços (RVB5-2-01). Parar, registrar no diário, e o Owner decide se o B5 segue. Para voltar, no *checkout* antigo: se só a linha 1 rodou, não há o que desfazer; se a linha 4 rodou, o Airbyte é novo — `make airbyte-up` → `make airbyte-config AUTO=1`, sobre o estado do Terraform já apartado —; e, da linha 2 em diante, `RESTAURAR=1 make recovery-restore`, com `make airflow-up` depois se a linha 3 rodou. Não basta a D50: o Airbyte novo recomeça as gerações, o bruto retido tem geração 1 (`brands`: mínima 1, no manifesto do candidato), e uma sincronização nova dividiria a geração com o job antigo — a certificação acusaria `inconsistent`. A sequência faz, na ordem, o re-base das gerações (D52, passo 4b), o contador (D50) e os disparos que a guarda pede — `sync-airbyte` antes de `sync-legacy`; e o *streaming* desmontado volta pelos passos 3 e 7. Devolver os bancos ao corte não perde nada, porque nada rodou depois dele (P4). **[planejado]** — é a sequência da linha 9, sobre bancos povoados |
...
| Técnico: migrações do zero | `migrate`, `migrate-legacy`; `governance.garantir()` no `dbt-build` | `make migrate-status` e `make migrate-legacy-status` nas fontes; no armazém, `set -a; . ./.env; set +a; .venv/bin/python -m mvp_ed1.governance versoes` → `0001_legacy_captures` e `0002_snapshot_id_e_o_job` |
| Técnico: testes passando | `make check` | `PASS=…`, `N passed`, e a lista de *skips* com motivo |
| Técnico: reconciliação entre camadas | dentro do `check` (os 14 testes da tag `fronteira`) + a comparação explícita dos caminhos | os 14 `pass`; os quatro zeros |
