...
|---|---|---|---|
| C1 | Todos os critérios de sucesso do Termo verificados **em ambiente limpo** | Tudo roda nesta máquina, sobre volumes que existem desde a Etapa 2 | Definir "limpo" (**D45**) e executar o ciclo da [Execução Local §3](docs/execucao_local.md#3-ciclo-completo) do zero, mapeando cada critério do Termo a um comando e uma saída |
...
| — | Artefato: versão marcada no Git | Nenhuma *tag* | Escolher o nome (**D47**) e marcar o *commit* que fecha M5 |
...
**Se achar algo (D48).** Um segredo de contêiner local (senha de `.env` gerada por `make env`)
não é a mesma coisa que uma chave de nuvem. A resposta está na §10.
...
**O que muda [planejado].** Três alvos, um módulo (`mvp_ed1.recovery`), um diretório (**D46**):
...
**Pré-condição:** B1–B4 entregues, pacote candidato montado e verificado, **D45** decidida.

**O que é "do zero" [planejado, conforme D45]:** clone novo em outro diretório (`git clone` de
`origin/main` na *tag* candidata), `.env` novo por `make env` (senhas novas — prova que nada
depende das antigas), `make install`, e **`make reset`** nos volumes desta máquina antes de
`make up`. O que **não** é zerado, e fica registrado: imagens Docker já baixadas, `abctl` e
Terraform em `.tools/`, o cluster `kind` do Airbyte (é `airbyte-down` + `airbyte-up`? — ver D45).
...
5. ***Tag*** anotada no *commit* de fechamento (**D47**), com a mensagem apontando para o pacote
...
- VM na nuvem como "ambiente limpo" — é o §4.1 da conversa de custos, intenção sua para depois
  da Etapa 13; se **D45** escolher, entra, mas não é o recomendado.
...
Na forma que você pediu: alternativas, e a recomendação com o motivo. Cada uma entra em
`docs/pendencias.md` como Dnn se você quiser tempo; se decidir aqui, fecho na próxima revisão do
plano e nenhum ADR novo é preciso — nenhuma delas troca ferramenta, camada ou modelagem.

### D45 — o que é "ambiente limpo"

| Opção | A favor | Contra |
|---|---|---|
| **(a) Clone novo + `.env` novo + `make reset` nesta máquina** — recomendada | Custo zero; é literalmente o que o Termo pede ("a partir do repositório"); imagens Docker em cache e `.tools/` não são estado do projeto — são o equivalente de ter o Docker instalado | Não prova que o repositório sobe numa máquina que nunca o viu (pré-requisitos da §1 da Execução Local, versão do Docker, do `kind`) |
...
| (c) (a) agora, (b) como critério da fase seguinte | O que (a) não prova fica registrado como contrapartida, como o ADR-0046 fez com a simultaneidade | Nenhum, além de escrever a frase |

**Recomendo (c)** — que na prática é (a), com a lacuna dita. Sub-decisão dentro de (a): o
cluster `kind` do Airbyte é derrubado (`airbyte-down`, cai na armadilha do `PG_VERSION` da §6 —
que, aliás, é bom testar) ou só pausado? Recomendo **derrubar**: "do zero" inclui o Airbyte, e a
armadilha documentada precisa ser reproduzida uma vez para valer como documentação.

### D46 — onde vive o pacote de recuperação

| Opção | A favor | Contra |
|---|---|---|
| **`data/recovery/`** (já ignorado pelo Git), com `RECOVERY_DIR` sobrescrevível no `.env` — recomendada | Zero configuração; a regra "não versionado" vale por construção; o caminho é o mesmo em toda máquina | Um `rm -rf data/` (que ninguém deveria fazer, mas o `.gitignore` convida) leva o pacote junto |
| Fora do repositório (`~/mvp_ed1_recovery/`) | Sobrevive ao repositório | Caminho por máquina; mais uma variável obrigatória |

### D47 — o nome da versão

| Opção | A favor | Contra |
|---|---|---|
| **`v1.0.0`** no *commit* que fecha M5 — recomendada | A fase local é um produto completo pelo Termo (E1–E11); a fase GCP é `v2.0.0` (E12 muda a plataforma, é *major*) | "1.0" sem nuvem pode parecer prematuro a quem lê o repositório sem o Termo |
...
1. **Deste plano, antes do código.** O que perguntar a ele: (i) algum critério do plano ou do
   ...
   que o GitHub resolve e a ferramenta não)? (v) as decisões da §10 têm alternativa que não vi?
