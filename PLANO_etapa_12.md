...
| P4 | O candidato do pacote foi montado com o código que o B5 vai usar, e é o estado dos bancos quando o B5 começa | `make recovery-pack`, depois `make recovery-verify CONTRA_O_BANCO=1`; o manifesto registra o `commit`; e **nada roda entre o `recovery-pack` e o §7.1** — nenhuma sincronização, DAG ou geração —, porque o recuo da fase 1 (§7.4) devolve os bancos ao corte | **[medido]** corte `2026-09-24T23:43:04Z`, `commit` `561edc2`, conferido contra os bancos. **Refazer depois da revisão deste roteiro**, com a ponta revisada: já há código depois de `561edc2` (o `-rs` do `make test`, `c9435d2`), e a revisão pode trazer mais. Nada compara o `commit` na restauração — ele é a procedência do pacote, e a do candidato promovido no passo 9 deve ser a ponta que o B5 clonou |
