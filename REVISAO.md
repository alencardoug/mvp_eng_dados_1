...
|---|---|---|---|---|
| RV10-2-01 | `src/mvp_ed1/legacy/captura.py:221`, `:344` | **O abandono pode revogar um certificado já publicado.** `_pendentes` é lido fora da transação de `_marcar`, e o UPDATE de abandono filtra só a tentativa. Se outro chamador registra o job ou conclui nesse intervalo, a retomada ainda escreve `abandoned` nas 40 linhas. EV10-2-02 reproduz `complete → abandoned` e a retirada da captura 902 das certificadas. Revalidar atomicamente `pending`, ausência de job e carência ao abandonar; testar a sobreposição entre leitura e escrita. | `bloqueante` | **Corrigido em 15/09/2026.** `_abandonar` é um `UPDATE` com a condição revalidada na escrita — `status = 'pending' and job_id is null and started_at <= agora − carência` — e só conta como abandono se gravou as 40 linhas; a lista velha não autoriza nada. Prova: `test_o_abandono_nao_sobrescreve_tentativa_que_ganhou_job_ou_foi_concluida` (lista de pendentes envelhecida por *monkeypatch*; B registra job e conclui; A devolve `ativa`, o `complete` fica e a captura continua certificada). Nota no ADR-0044, itens 4–5. |
| RV10-2-02 | `src/mvp_ed1/legacy/captura.py:278`, `:302` | **Uma conclusão concorrente devolve um certificado que não foi persistido.** A guarda `status = 'pending'` protege a escrita vencedora, mas `_publicar` não informa se gravou e `concluir` devolve os vereditos calculados antes da disputa. EV10-2-02 obtém `complete/snapshot 903` com 40 linhas `unstable` no banco. O chamador recebe autorização para avançar incompatível com a fonte de elegibilidade. Devolver o resultado efetivamente persistido, inclusive quando outro chamador vence, e provar os dois sentidos da disputa. | `bloqueante` | **Corrigido em 15/09/2026.** `concluir` devolve `_gravado(...)` — o certificado como está no banco — depois de publicar, ganhe ou perca a disputa; `PENDING` entra em `_pior` para o caso em que nada foi gravado. Prova: `test_a_conclusao_que_perde_a_disputa_devolve_o_certificado_gravado`, nos dois sentidos (A mede `complete`, B publica `unstable` antes → A devolve `unstable`; A publica `complete`, B mediu a origem alterada → B devolve `complete`), com o segundo chamador interposto dentro de `medir_origem` do primeiro. |
| RV10-2-03 | `src/mvp_ed1/legacy/captura.py:196`, `:260`; `airflow/dags/fluxo_batch.py:97` | **Reassociar uma tentativa fechada a outro job mistura duas capturas.** `registrar_job` sobrescreve qualquer tentativa; o novo retorno antecipado de `concluir` conserva o certificado antigo. Reexecutar a tarefa de sincronização com a mesma tentativa pode, assim, devolver `complete` para o novo job com o snapshot antigo. EV10-2-02 mede `job_id=999`, `snapshot_id=901`, `complete` nas 40 linhas. Tornar a associação idempotente para o mesmo job e impedir a troca de job numa tentativa já associada/fechada; a nova sincronização precisa de sua própria fase 1. | `bloqueante` | **Corrigido em 15/09/2026.** `registrar_job` é um `UPDATE` com `((job_id is null and status = 'pending') or job_id = :j)`; se não alcança as 40 linhas lança `TentativaIndisponivel`. Idempotente para o mesmo job, antes e depois de fechada; recusa outro job. A DAG documenta que reexecutar a sincronização depois da fase 2 exige execução nova. Prova: `test_a_associacao_do_job_e_idempotente_e_recusa_outro_job` (914 duas vezes; 915 recusado; 999 recusado depois do `complete`; `job_id`/`snapshot_id` permanecem 914). |
| RV10-2-04 | `dbt/macros/chave_canonica.sql:52`, `:54` | **A guarda inteira ainda deixa entradas inválidas derrubarem o modelo.** `U+2003 + 8 + U+2003` passa pelo regex e lança `22P02` no `::numeric`; 131.073 dígitos lançam `22003` nessa conversão antes da conferência do domínio. Ambos deveriam produzir nulo, pois o cast ao tipo declarado os recusa (EV10-2-03). Fazer a guarda/conversão ser segura em todo o domínio textual, incluindo espaços Unicode e entradas extensas, e exercitar a macro real nos dois bancos. | `bloqueante` | **Corrigido em 15/09/2026.** A guarda passa a aceitar só os brancos ASCII do `isspace` (`[ \t\n\v\f\r]`), os zeros à esquerda saem por `regexp_replace` antes de qualquer conversão, e o `::numeric` só roda sobre ≤ 19 dígitos significativos — nunca sobre a entrada inteira. Prova: `test_a_macro_real_devolve_o_que_o_cast_do_postgresql_devolve_ou_nulo`, agora nos dois bancos (`legacy_db` e `warehouse_db`), cada conversão na sua transação, com U+2003, U+00A0, TAB/LF/VT/FF, 5.000 zeros + 8, 131.080 zeros, 25 e 131.073 noves — a macro nunca lança e coincide com o `cast` ou dá nulo onde ele lança. |
| RV10-2-05 | `src/mvp_ed1/legacy/remocao.py:47`, `:88`, `:94` | **O espelho Python não reproduz a fronteira do PostgreSQL.** NBSP em volta de `8` vira identidade em Python e nulo na macro; UUID seguido de newline lança `ValueError` porque `match` com `$` aceita a quebra final; 5.000 zeros seguidos de `8` convertem no banco e lançam no `int` Python (EV10-2-03). A declaração do esperado do diário pode fabricar identidade ou interromper a prova. Validar a string inteira e o domínio antes da conversão, respeitando os espaços aceitos pelo cast e sem depender do limite de dígitos do interpretador. | `bloqueante` | **Corrigido em 15/09/2026.** `remocao.canonizar` usa os mesmos regex com `\Z` (o `$` do Python aceitava `\n` final) e brancos ASCII explícitos; tira os zeros à esquerda pelo próprio regex e recusa mais de 19 dígitos significativos antes do `int()` — o limite vem do domínio, não do interpretador. Prova: o mesmo teste da macro compara `canonizar` ao `cast` em todas as formas (UUID com `\n`, NBSP, U+2003, 5.000 zeros, 131.073 dígitos). |
| RV10-2-06 | `src/mvp_ed1/legacy/mutacoes.py:71`; `:195` | **Alterar a própria PK continua produzindo esperado com a chave antiga.** A CLI permite escolher a PK como coluna alterada e o `RETURNING` entrega seu novo valor, mas `efeito_liquido` usa `valor_da_chave`. No PostgreSQL efêmero, `id='1' → '2'` deixa somente `'2'` na origem e o esperado declara `'1'` presente (EV10-2-04). Derivar a identidade nova do retorno e contabilizar corretamente o efeito sobre a identidade anterior. | `bloqueante` | **Corrigido em 15/09/2026.** Cada mutação grava `presenca_apos_commit`: quantas linhas físicas restam para cada chave **canônica** tocada — em `alterar`, a antiga e, quando a coluna é a PK, as novas devolvidas. `efeito_liquido` deriva daí. Prova: `test_o_diario_registra_a_multiplicidade_canonica_depois_do_commit` em PostgreSQL efêmero (`'1' → '2'` grava `{1: 0, 2: 2}`; o efeito diz `1` ausente, `2` presente). |
| RV10-2-07 | `src/mvp_ed1/legacy/mutacoes.py:66`, `:68` | **Apagar um alias textual não prova ausência da chave canônica.** Com ocorrências `'8'` e `'08'`, remover só `'08'` devolve uma linha e deixa `'8'` no banco; o esperado sobrescreve a chave canônica `8` como ausente. A confirmação pós-commit verifica as chaves textuais solicitadas, portanto seu zero não prova multiplicidade canônica zero (EV10-2-04). Calcular o efeito a partir das ocorrências efetivas e sua identidade canônica, preservando as sobreviventes; incluir redução sem remoção no teste do diário. | `bloqueante` | **Corrigido em 15/09/2026**, pelo mesmo mecanismo: apagar só `'08'` com `'8'` sobrevivente grava `{8: 1}` — redução, não remoção — e o efeito diz a chave `8` presente. `_presenca_canonica` lê a coluna da PK inteira e canoniza em Python, porque a igualdade textual da CLI não vê aliases. Entradas do diário anteriores ao campo (as 16 de 15/09) continuam lidas pelo `RETURNING` canonizado, e o teste unitário cobre as duas leituras. |
| RV10-2-08 | `src/mvp_ed1/legacy/oraculo.py:493`; `tests/test_legado_contraprovas.py:251` | **O comparador de recuperação aceita inteiro corrompido em fração.** `int(Decimal(...))` trunca os dois lados. A projeção SQL mutada devolve `244.9` para o esperado `244`, e `_divergencias` continua vazio (EV10-2-05). O helper foi movido do teste antigo e passou a sustentar também a contraprova nova. Comparar o valor numérico sem truncamento e exigir integralidade; acrescentar esse mutante à prova sobre SQL executado. | `ajuste` | **Corrigido em 15/09/2026.** `oraculo.mesmo_valor` para inteiro exige igualdade numérica **e** integralidade (`Decimal(obtido) == to_integral_value()`), sem truncar. O mutante entrou na contraprova (b) executada: projeção de `quantity_ordered` em `purchase_order_items` com `|| '.9'` → acusado no campo `valor`. |
| RV10-2-09 | `tests/test_legado_contraprovas.py:182`, `:186` | **A contraprova anunciada de precedência aceita não detectar nada.** O lote usado não tem a sobreposição relevante: inverter o catálogo produz zero divergências, e `or divergencias == []` aprova exatamente isso (EV10-2-05). Cascata e origem da rejeição foram exercitadas, mas a afirmação de que a mutação de precedência divergiu não procede. Usar uma sobreposição que diferencie as ordens — como o caso de RV10-06 — e exigir a divergência correspondente. | `ajuste` | **Corrigido em 15/09/2026.** A contraprova (a) agora afirma que o lote gerado **não** distingue as ordens (zero divergências com o catálogo invertido) e prova a precedência num lote com a sobreposição literal de RV10-06 (`_lote_com_sobreposicao`): com a ordem certa `rejected`/`TEXT_TRUNCATED`, invertida `corrected`/`TEXT_WHITESPACE_CASE`, e a comparação acusa `classification` e `achados`. O `or divergencias == []` saiu. |
| RV10-2-10 | `docs/execucao_local.md:269`; `docs/capacidade_e_recuperacao.md:169`, `:193`; `README.md:154` | **A propagação do ADR-0046 deixou a orientação antiga em vigor nos mesmos documentos.** Execução Local ainda apresenta a Etapa 12 como uso de `FORCE=1` e D36 como pendente; Capacidade mantém a exceção de simultaneidade na validação final, também mencionada no README (EV10-2-07). Essas instruções contradizem o critério novo e podem levar o operador ao cenário que o Owner retirou. Atualizar as orientações vigentes e referências, preservando números históricos e ADRs aceitos. | `ajuste` | **Corrigido em 15/09/2026.** Execução Local §5 (recusa/`FORCE=1`), Capacidade §2.4, §2.5 e §2.8 e README (caminho quente) passam a dizer que nenhuma etapa exige tudo de pé — a Etapa 12 valida por partes (ADR-0046); os números históricos ficam. O README também deixa de dizer que a v9 "chegará": está no armazém desde o *build* da noite de 15/09, com as auditorias v8/v9 iguais linha a linha (medido pelo revisor, EV10-2-06). |

### Resposta do desenvolvimento — 15/09/2026, segunda rodada

Dez achados, dez corrigidos; nenhuma decisão do Owner reaberta. A coluna *Situação* de cada linha
diz o que mudou e onde está a prova. Cinco *commits*: concorrência do certificado (RV10-2-01/02/03),
fronteira da chave canônica nos dois lados (04/05), multiplicidade canônica no diário (06/07),
comparador e contraprova de precedência (08/09), propagação documental do ADR-0046 e v9 no
README (10).

**Medido**, saída literal:

`make test` (sem `FATO`, `CARGA`) — oito testes novos; os oito pulados são os mesmos:

```text
224 passed, 8 skipped in 155.69s (0:02:35)
```

`make dbt-build DBT_ARGS='--select legacy_presence_by_capture+'` — os três modelos da exclusão
física com a macro corrigida, sobre as oito capturas certificadas do armazém de trabalho; o mesmo
resultado de antes (5 na memória, 12.738 `mantida` no intervalo 35→36):

```text
1 of 27 OK created sql table model trusted.legacy_presence_by_capture .......... [SELECT 101897 in 4.82s]
11 of 27 OK created sql table model trusted.legacy_removed_records ............. [SELECT 5 in 0.48s]
12 of 27 OK created sql table model trusted.legacy_capture_transitions ......... [SELECT 12738 in 0.36s]
Done. PASS=27 WARN=0 ERROR=0 SKIP=0 NO-OP=0 REUSED=0 TOTAL=27
```

**Não verificado nesta resposta:** `make dbt-build` completo; a DAG com a guarda nova de
`registrar_job` (só exercitada em banco efêmero); `FATO=1`/`CARGA=1`; interrupção de *job* real. O
diário de produção (16 entradas) não tem `presenca_apos_commit` — continua lido pelo `RETURNING`
canonizado, e o teste de integração do ciclo real passou assim; entradas novas terão o campo.
