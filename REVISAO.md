...
| R13 | Testes do legado e manifesto | Há testes sintéticos úteis e eles passaram; ainda falta o esperado independente completo por ocorrência para o lote, a cascata e os valores recuperados. A paridade dos arquivos e a cobertura de códigos não fecham essa prova. | `ajuste` | **Pendente.** Nova injeção passou nos testes do gerador; detecção integral no banco não medida devido aos timeouts. |
