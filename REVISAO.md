...
|---|---|---|---|---|
| R01 | `dbt/models/trusted/legacy/legacy_eligible_records.sql:1`; `README.md:85`; ADR-0039 | **O empilhamento e a procedência até analytics não foram entregues.** O conjunto apto não tem consumidor em modelos/snapshots; por exemplo, `trusted.order_items` ainda lê somente `stg_retail__order_items`, e a chave da fato de venda continua derivada apenas do id do item. Portanto os relatórios não incluem o legado e `empilhados = aceitos + corrigidos` não foi demonstrada na fronteira real. Concluir o empilhamento com as chaves e o alcance do ADR-0039, incluindo testes de colisão/reconciliação, antes de declarar a Etapa 10 concluída. Não basta acrescentar um `union all` sem adequar as chaves. | `bloqueante` | **Fatia entregue e verificada; o restante continua aberto.** `customers` atravessa até `analytics`: 74 clientes do legado empilhados ao lado de 1.500 da origem principal, com `source_system` na coluna e na chave substituta (ADR-0039). A colisão era real e apareceu na primeira construção — os **74 identificadores existem nas duas origens**, e o `unique` de `dim_customer` quebrou antes de a chave mudar. Três testes novos guardam: reconciliação na fronteira do empilhamento, ausência de colisão, e paridade de formato entre a ponte e o `staging`. **Faltam as demais tabelas** — o padrão está provado, o trabalho não está feito. |
