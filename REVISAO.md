...
| R09 | `airflow/dags/fluxo_batch.py:93`; seleção da captura | A docstring agora admite a lacuna, mas existência de linhas nos streams não comprova que a captura é integral ou pertence ao job concluído. Falta a prova que distingue captura anterior reutilizada de carga nova. | `bloqueante` | **Implementado e medido em 14–15/09/2026, aguardando revisão** ([ADR-0044](docs/adr/0044-certificar-cada-captura-do-legado-por-conteudo.md)). Cada sincronização produz certificado por *stream* em `governance.legacy_captures`, em duas fases: contagem e hash de conteúdo da origem **antes** do *job*, origem e bruto **depois**, `_airbyte_meta.sync_id` conferido linha a linha; só `complete` nas 40 tabelas é elegível, e a seleção passou a ser a mais recente certificada. Medido: capturas 28–33, 35 e 36 `complete` (a 36 nascida dentro da DAG, 12 tarefas em 13 min); captura 34 com *stream* desabilitado — `succeeded` no Airbyte, 12.728 linhas — `incomplete` em `coupon_redemptions` (origem 9, recebido 0), não selecionada, recusada quando forçada; geração 15 retida: 39 tabelas iguais à 16, `brands` ausente. Achado no caminho: `_airbyte_generation_id` é por *stream* — a identidade da captura passou a ser o *job* (v8). |
| R10 | Comparação entre capturas; ADR-0037 | A detecção de exclusões físicas entre capturas completas continua ausente. Retenção e presença de stream não a substituem. | `bloqueante` | **Implementado e medido em 15/09/2026, aguardando revisão** ([ADR-0045](docs/adr/0045-detectar-exclusao-fisica-do-legado-no-bruto-retido.md)). Detecção só no bruto retido, pela PK declarada canonizada pelo tipo, entre capturas certificadas: `legacy_capture_transitions` (intervalo, com multiplicidade) e `legacy_removed_records` (memória, persistente por construção); equação em linhas físicas como teste; sem marca dimensional (decisão do Owner: sob 0038/0042 a remoção já cascateia para fora das fatos). Medido entre capturas reais, com esperado no diário do manifesto (`RETURNING` + hash antes/depois): B — 5 clientes e 3 movimentos `removida`, 1 `adicionada`, cliente inválido `mantida`, os 3 movimentos saíram da fato (503 → 500) e 7 pedidos viraram `FK_ORPHAN`; C — sem mudança, 12.735 `mantida`, memória intacta; D — reaparecimento (`adicionada`, sai da memória); D′ — nova exclusão (`last_seen 31, removed_in 32`); E — tabela esvaziada certifica `complete` e as 6 linhas são `removida`; F — captura incompleta não comparada. Cinco *unit tests* dbt. |
| R12 | `src/mvp_ed1/legacy/writer.py`, `schema.py`; Alembic | O schema da origem legada continua criado por DDL fora do ciclo versionado de evolução/reversão. | `bloqueante` | **Implementado e medido em 14/09/2026, aguardando revisão.** `db/migrations_legacy/` (`alembic -n legacy`), revisão inicial por *autogenerate* em banco isolado; equivalência física tripla — migrado × `ddl()` de referência × `legacy_db` — com **zero diferenças** antes do `stamp`; `downgrade base` → 0 tabelas e sem schema; `upgrade` de novo → 40; `alembic check` limpo; a carga recusa banco fora da cabeça (`SchemaNaoMigrado`). Testes em bancos efêmeros a cada `make test`. |
| R13 | Testes do legado e manifesto | Há testes sintéticos úteis e eles passaram; ainda falta o esperado independente completo por ocorrência para o lote, a cascata e os valores recuperados. A paridade dos arquivos e a cobertura de códigos não fecham essa prova. | `ajuste` | **Implementado e medido em 14–15/09/2026, aguardando revisão.** `legacy/oraculo.py` calcula, para **toda** ocorrência do lote, saída, origem da rejeição e o multiconjunto de achados — contexto (obrigatoriedade, duplicatas, órfãos, total do pedido) e cascata recomputados das mutações do injetor e do grafo declarado, sem ler o classificador; o valor esperado de cada correção vem do contrato `recuperacao` do catálogo; o manifesto é identificado por hash de conteúdo e a captura é conferida contra ele antes de comparar. Medido sobre a captura 28 (v8): **12.747 vereditos iguais, 0 divergências**; 44 valores recuperados conforme o contrato, comparação tipada; 82/82 achados de valor; 11 falsos positivos da heurística em 12.747 linhas. O oráculo achou dois defeitos antes de bater: a cópia de duplicata que herdava o defeito sem o achado, e a própria heurística de truncamento, que ele não conhecia (2.271 divergências até aplicar o contrato do catálogo). Contraprovas por mutação no esperado, no obtido, na multiplicidade e no conteúdo. |
| R14 | `README.md`; `docs/pendencias.md`; Plano §Etapa 10 | Permanecem estados incompatíveis: empilhamento declarado inexistente e descrito como implementado; etapa reaberta e etapas 3–10 anunciadas entregues; pendências resumidas a R10/R12/R13. Identificar medidas por captura/versão e atualizar o estado nos donos documentais, preservando ADRs aceitos. | `ajuste` | **Tratado em 15/09/2026, aguardando revisão.** README, Pendências, Plano §Etapa 10 e Origem Legada §4.1/§4.2/§6 descrevem o estado **medido**, com captura e versão: captura 36, v8, 16/16 views, fato 16.403, `make dbt-build` `PASS=891`, DAG 12 tarefas em 13 min; as observações anteriores ficam datadas como anteriores às reconstruções. Nenhum documento declara a etapa aceita — todos dizem "aguardando revisão". |
...
| R25 | `dbt/models/analytics/fact_inventory_movement.sql:40` | A captura completa do legado ainda alimenta o filtro incremental global de 7 dias; aptidão nova fora da janela e rejeição/remoção posterior não são reconciliadas pela estratégia atual. Provar atualização e remoção no reprocessamento sem assumir que reconstrução inicial basta. | `bloqueante` | **Resolvido.** A decisão voltou do Owner como [ADR-0042](docs/adr/0042-reconciliar-a-captura-legada-na-fato-incremental.md) e foi implementada: o ramo legado da fato entra por `delete+insert` — `pre_hook` apaga a partição, o `select` a reinsere pela captura corrente —, e a marca d'água da janela passou a ser lida das linhas de `retail`, não do máximo global que arrastava o legado para fora. **A prova é de reprocessamento, não de reconstrução**, que era o cerne do achado: `tests/test_fato_incremental.py` estraga a partição de propósito nos dois caminhos e manda reprocessar sem `--full-refresh`. Contraprova de que o teste discrimina: com a estratégia antiga restaurada, o movimento ausente da captura sobrevive (1 em vez de 0) e a divergência fora da janela não é corrigida (2390 em vez de 390); com a nova, os dois são reparados. Somam-se o teste por linha `legado_na_fato_segue_a_captura_corrente` e a reconciliação agregada, que voltou a fechar: 16.453 = 16.453 linhas, saldo 729.150 = 729.150. |
| R26 | `src/mvp_ed1/legacy/ponte.py:68` | O contrato de `event_sequence = legacy_row_id` continua dependente do esclarecimento já solicitado. Identidade física da captura não demonstra ordem observada ou estabilidade entre recapturas. | `observação` | **Esclarecido pelo Owner em 14/09/2026 (D40).** `event_sequence = legacy_row_id` é desempate técnico dentro da captura, único por origem, e nada além — sem ordem observada do evento nem estabilidade entre recapturas. Nota de referência no ADR-0039, dono em Origem Legada §4.1, comentário na ponte, e o teste `legado_event_sequence_e_desempate_tecnico` cobra só o contrato (PASS). |
