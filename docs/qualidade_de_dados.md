...
|---|---|---|
| `oltp` → `raw` | Contagem por tabela e por lote | `tests/test_reconciliacao_raw.py`: o conjunto de identidades de cada tabela em `raw` é igual ao da origem no instante da extração, comparado **nos dois sentidos** — identidade da origem ausente é `raw` atrás; identidade sem origem é sobra de regeneração — porque a entrega é ao menos uma vez e uma perda compensada por uma sobra fecharia a contagem; e toda linha pertence a um *job* identificado, com a contagem por lote registrada |
