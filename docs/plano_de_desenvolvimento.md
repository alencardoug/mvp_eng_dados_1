...
|---|---|---|
| **M0** | Termo de Abertura aprovado e documentação-base estável | Etapa 0 |
| **M1** | Decisões fundamentais registradas em ADR | Etapa 1 |
...
| **Critérios de conclusão** | Termo revisado e aprovado pelo Owner · nenhuma consideração duplicada entre documentos · `.gitignore` revisado · ADR-0001 a ADR-0007 aceitos |
...
| **Decisões** | **D10** (estrutura de diretórios) |
...
| **Decisões** | **D20**, **D21** (carga completa × incremental, exclusões), **D23** (materializações), **D24** (medidas), **D25** (chaves substitutas e SCD), **D27** (primeiras views) |
...
| **Decisões** | **D16** (materialização do saldo), **D17** (entrega, latência e janela), **D18** (cursor de CDC) |
...
| **Decisões** | **D15** (chave de procedência), **D28** (dicionário de conversões e motivos de rejeição) |
...
| **Entregas** | **E8**, **E9**, **E10** |
| **Decisões** | **D14** (escopo do schema `governance`) |
...
| **Entregas** | **E12** |
| **Decisões** | **D11** (Airbyte no GCP), **D22** (execução do Airflow) |
...
| **Critérios de conclusão** | Todo item do [mapa de paridade](arquitetura.md#5-mapa-de-paridade-local--gcp) com equivalente provisionado · `terraform plan` revisado antes de cada `apply` · particionamento, *clustering*, retenção e políticas definidos antes do provisionamento · *policy tag* aplicada a cada coluna sensível, com acesso negado comprovado · paridade funcional com a fase local demonstrada |
