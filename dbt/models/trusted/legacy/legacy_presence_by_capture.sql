...
select
    ...
    (r._airbyte_meta->>'sync_id')::bigint                     as snapshot_id,
    {{ chave_canonica('r."id"', 'bigint') }}                  as business_key,
    count(*)                                                  as n,
...
where (r._airbyte_meta->>'sync_id')::bigint in (select snapshot_id from consideradas)
group by 1, 2, 3
