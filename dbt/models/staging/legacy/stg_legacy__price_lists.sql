...
-- Limpeza de `legacy.price_lists` — camada `staging` (ADR-0016).
--
...
with captura as (
    ...
    select *
    from {{ source('legacy', 'price_lists') }}
    where _airbyte_generation_id = coalesce(
        {{ legacy_snapshot_id() }}, (
        select max(_airbyte_generation_id) from {{ source('legacy', 'price_lists') }}
    ))
...
select
    ...
    jsonb_build_object('id', c."id", 'code', c."code", 'name', c."name", 'sales_channel_id', c."sales_channel_id", 'currency', c."currency", 'valid_from', c."valid_from", 'valid_to', c."valid_to", 'is_active', c."is_active", 'created_at', c."created_at", 'updated_at', c."updated_at", 'deleted_at', c."deleted_at")              as original_payload
from captura c
