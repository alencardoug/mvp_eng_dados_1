...
select
    ...
    jsonb_strip_nulls(
        jsonb_build_object(
            ...
            'movement_type', case
            when btrim(l."movement_type") not in ('purchase_receipt', 'customer_return', 'transfer_in', 'adjustment_in', 'sale_dispatch', 'supplier_return', 'transfer_out', 'adjustment_out') then 'ENUM_UNKNOWN'
            ...
            'quantity_delta', case
            when l."quantity_delta" ~ '^-?[0-9]+$' and abs(l."quantity_delta"::numeric) > 1000000 then 'NUM_OUT_OF_RANGE'
            when l."quantity_delta" !~ '^-?[0-9]+$' then 'NUM_AMBIGUOUS'
            ...
            'unit_cost', case
            when btrim(replace(replace(l."unit_cost", 'R$', ''), ' ', '')) ~ '^-' then 'MONEY_NEGATIVE'
            when l."unit_cost" !~ '^-?[0-9]+(\.[0-9]{1,4})?$' then 'MONEY_AMBIGUOUS'
            ...
            'source_type', case
            when btrim(l."source_type") not in ('purchase', 'sale', 'return', 'transfer', 'adjustment') then 'ENUM_UNKNOWN'
            ...
            'recorded_at', case
            when case when l."recorded_at" ~ '^[0-9]{4}[-.][0-9]{2}[-.][0-9]{2}'   then not (substring(l."recorded_at" from 6 for 2)::int between 1 and 12 and substring(l."recorded_at" from 9 for 2)::int between 1 and 31 and not (substring(l."recorded_at" from 6 for 2)::int in (4, 6, 9, 11) and substring(l."recorded_at" from 9 for 2)::int > 30) and not (substring(l."recorded_at" from 6 for 2)::int = 2 and substring(l."recorded_at" from 9 for 2)::int > 29) and not (substring(l."recorded_at" from 6 for 2)::int = 2 and substring(l."recorded_at" from 9 for 2)::int = 29 and not (substring(l."recorded_at" from 1 for 4)::int % 4 = 0 and (substring(l."recorded_at" from 1 for 4)::int % 100 <> 0 or substring(l."recorded_at" from 1 for 4)::int % 400 = 0)))) when l."recorded_at" ~ '^[0-9]{2}/[0-9]{2}/[0-9]{4}$'   then not (substring(l."recorded_at" from 4 for 2)::int between 1 and 12 and substring(l."recorded_at" from 1 for 2)::int between 1 and 31 and not (substring(l."recorded_at" from 4 for 2)::int in (4, 6, 9, 11) and substring(l."recorded_at" from 1 for 2)::int > 30) and not (substring(l."recorded_at" from 4 for 2)::int = 2 and substring(l."recorded_at" from 1 for 2)::int > 29) and not (substring(l."recorded_at" from 4 for 2)::int = 2 and substring(l."recorded_at" from 1 for 2)::int = 29 and not (substring(l."recorded_at" from 7 for 4)::int % 4 = 0 and (substring(l."recorded_at" from 7 for 4)::int % 100 <> 0 or substring(l."recorded_at" from 7 for 4)::int % 400 = 0)))) when l."recorded_at" ~ '^[0-9]{2}/[0-9]{4}$' then true else false end then 'DATE_IMPOSSIBLE'
            when case when l."recorded_at" ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}' and (substring(l."recorded_at" from 6 for 2)::int between 1 and 12 and substring(l."recorded_at" from 9 for 2)::int between 1 and 31 and not (substring(l."recorded_at" from 6 for 2)::int in (4, 6, 9, 11) and substring(l."recorded_at" from 9 for 2)::int > 30) and not (substring(l."recorded_at" from 6 for 2)::int = 2 and substring(l."recorded_at" from 9 for 2)::int > 29) and not (substring(l."recorded_at" from 6 for 2)::int = 2 and substring(l."recorded_at" from 9 for 2)::int = 29 and not (substring(l."recorded_at" from 1 for 4)::int % 4 = 0 and (substring(l."recorded_at" from 1 for 4)::int % 100 <> 0 or substring(l."recorded_at" from 1 for 4)::int % 400 = 0)))) then left(l."recorded_at", 10)::date > date '{{ var("as_of_date") }}' else false end then 'DATE_FUTURE'
            when l."recorded_at" !~ '^[0-9]{4}[-.][0-9]{2}[-.][0-9]{2}' and l."recorded_at" !~ '^[0-9]{2}/[0-9]{2}/[0-9]{4}$' and l."recorded_at" !~ '^[0-9]{2}/[0-9]{4}$' then 'DATE_UNPARSEABLE'
