...
select
    ...
    -- ── Valores tratados ─────────────────────────────────────────────────────
        ...
        case
            ...
            when c."tracking_url_template" like '%;%' then c."tracking_url_template"
            when c."tracking_url_template" ~ '(Ã.|Â.)' then convert_from(convert_to(c."tracking_url_template", 'LATIN1'), 'UTF8')
    ...
    jsonb_strip_nulls(
        jsonb_build_object(
            ...
            when c."tracking_url_template" like '%;%' then 'TEXT_DELIMITER'
            when c."tracking_url_template" ~ '(Ã.|Â.)' then 'TEXT_ENCODING'
