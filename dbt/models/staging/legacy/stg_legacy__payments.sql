...
-- Limpeza de `legacy.payments` — camada `staging` (ADR-0016).
--
...
with captura as (
    ...
    where _airbyte_generation_id = coalesce(
        {{ legacy_snapshot_id() }}, (
        select max(_airbyte_generation_id) from {{ source('legacy', 'payments') }}
    ))
...
select
    ...
    -- ── Valores tratados ─────────────────────────────────────────────────────
        ...
        case
            when btrim(c."payment_code") = '' or btrim(c."payment_code") in ('NULL', 'null', 'N/A', '-', '#N/D', '   ') then null
            when c."payment_code" like '%;%' then c."payment_code"
            when c."payment_code" ~ '(Ã.|Â.)' then convert_from(convert_to(c."payment_code", 'LATIN1'), 'UTF8')
            when c."payment_code" <> btrim(c."payment_code") or c."payment_code" ~ '  ' then regexp_replace(btrim(c."payment_code"), '\s+', ' ', 'g')
            else c."payment_code"
        end as "payment_code",
        c."order_id" as "order_id",
        c."payment_method_id" as "payment_method_id",
        case
            when btrim(c."status") = '' or btrim(c."status") in ('NULL', 'null', 'N/A', '-', '#N/D', '   ') then null
            when btrim(c."status") not in ('pending', 'authorized', 'captured', 'failed', 'refunded', 'cancelled') then c."status"
            else c."status"
        end as "status",
        case
        ...
        case
            when btrim(c."installments") = '' or btrim(c."installments") in ('NULL', 'null', 'N/A', '-', '#N/D', '   ') then null
            else c."installments"
        end as "installments",
        case
            when btrim(c."authorized_at") = '' or btrim(c."authorized_at") in ('NULL', 'null', 'N/A', '-', '#N/D', '   ') then null
            when c."authorized_at" !~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}' and ( c."authorized_at" ~ '^[0-9]{2}/[0-9]{4}$' or (c."authorized_at" ~ '^[0-9]{2}/[0-9]{2}/[0-9]{4}$' and (   substring(c."authorized_at" from 4 for 2)::int not between 1 and 12   or substring(c."authorized_at" from 1 for 2)::int not between 1 and 31   or (substring(c."authorized_at" from 4 for 2)::int in (4, 6, 9, 11)       and substring(c."authorized_at" from 1 for 2)::int > 30)   or (substring(c."authorized_at" from 4 for 2)::int = 2       and substring(c."authorized_at" from 1 for 2)::int > 29)   or (substring(c."authorized_at" from 4 for 2)::int = 2       and substring(c."authorized_at" from 1 for 2)::int = 29       and not (substring(c."authorized_at" from 7 for 4)::int % 4 = 0                and (substring(c."authorized_at" from 7 for 4)::int % 100 <> 0                     or substring(c."authorized_at" from 7 for 4)::int % 400 = 0)))))) then c."authorized_at"
            when c."authorized_at" ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}' and left(c."authorized_at", 10)::date > date '{{ var("as_of_date") }}' then c."authorized_at"
            when c."authorized_at" !~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}' and ( (c."authorized_at" ~ '^[0-9]{2}/[0-9]{2}/[0-9]{4}$'  and substring(c."authorized_at" from 4 for 2)::int between 1 and 12  and substring(c."authorized_at" from 1 for 2)::int between 1 and 31) or c."authorized_at" ~ '^[0-9]{4}\.[0-9]{2}\.[0-9]{2}$') then case when c."authorized_at" ~ '^[0-9]{2}/' then to_date(c."authorized_at", 'DD/MM/YYYY')::text else to_date(c."authorized_at", 'YYYY.MM.DD')::text end
            when c."authorized_at" ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}[ T][0-9]{2}:[0-9]{2}' and c."authorized_at" !~ '(Z|[+-][0-9]{2}:?[0-9]{2})$' then (c."authorized_at"::timestamp at time zone 'America/Sao_Paulo')::text
            else c."authorized_at"
        end as "authorized_at",
        case
            when btrim(c."captured_at") = '' or btrim(c."captured_at") in ('NULL', 'null', 'N/A', '-', '#N/D', '   ') then null
            when c."captured_at" !~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}' and ( c."captured_at" ~ '^[0-9]{2}/[0-9]{4}$' or (c."captured_at" ~ '^[0-9]{2}/[0-9]{2}/[0-9]{4}$' and (   substring(c."captured_at" from 4 for 2)::int not between 1 and 12   or substring(c."captured_at" from 1 for 2)::int not between 1 and 31   or (substring(c."captured_at" from 4 for 2)::int in (4, 6, 9, 11)       and substring(c."captured_at" from 1 for 2)::int > 30)   or (substring(c."captured_at" from 4 for 2)::int = 2       and substring(c."captured_at" from 1 for 2)::int > 29)   or (substring(c."captured_at" from 4 for 2)::int = 2       and substring(c."captured_at" from 1 for 2)::int = 29       and not (substring(c."captured_at" from 7 for 4)::int % 4 = 0                and (substring(c."captured_at" from 7 for 4)::int % 100 <> 0                     or substring(c."captured_at" from 7 for 4)::int % 400 = 0)))))) then c."captured_at"
            when c."captured_at" ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}' and left(c."captured_at", 10)::date > date '{{ var("as_of_date") }}' then c."captured_at"
            when c."captured_at" !~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}' and ( (c."captured_at" ~ '^[0-9]{2}/[0-9]{2}/[0-9]{4}$'  and substring(c."captured_at" from 4 for 2)::int between 1 and 12  and substring(c."captured_at" from 1 for 2)::int between 1 and 31) or c."captured_at" ~ '^[0-9]{4}\.[0-9]{2}\.[0-9]{2}$') then case when c."captured_at" ~ '^[0-9]{2}/' then to_date(c."captured_at", 'DD/MM/YYYY')::text else to_date(c."captured_at", 'YYYY.MM.DD')::text end
            when c."captured_at" ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}[ T][0-9]{2}:[0-9]{2}' and c."captured_at" !~ '(Z|[+-][0-9]{2}:?[0-9]{2})$' then (c."captured_at"::timestamp at time zone 'America/Sao_Paulo')::text
            else c."captured_at"
        end as "captured_at",
        case
    ...
    jsonb_strip_nulls(
        jsonb_build_object(
            ...
            'payment_code', case
            when btrim(c."payment_code") = '' or btrim(c."payment_code") in ('NULL', 'null', 'N/A', '-', '#N/D', '   ') then 'NULL_DISGUISED'
            when c."payment_code" like '%;%' then 'TEXT_DELIMITER'
            when c."payment_code" ~ '(Ã.|Â.)' then 'TEXT_ENCODING'
            when c."payment_code" <> btrim(c."payment_code") or c."payment_code" ~ '  ' then 'TEXT_WHITESPACE_CASE'
        end,
            'status', case
            when btrim(c."status") = '' or btrim(c."status") in ('NULL', 'null', 'N/A', '-', '#N/D', '   ') then 'NULL_DISGUISED'
            when btrim(c."status") not in ('pending', 'authorized', 'captured', 'failed', 'refunded', 'cancelled') then 'ENUM_UNKNOWN'
        end,
            'amount', case
            ...
            'installments', case
            when btrim(c."installments") = '' or btrim(c."installments") in ('NULL', 'null', 'N/A', '-', '#N/D', '   ') then 'NULL_DISGUISED'
        end,
            'authorized_at', case
            when btrim(c."authorized_at") = '' or btrim(c."authorized_at") in ('NULL', 'null', 'N/A', '-', '#N/D', '   ') then 'NULL_DISGUISED'
            when c."authorized_at" !~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}' and ( c."authorized_at" ~ '^[0-9]{2}/[0-9]{4}$' or (c."authorized_at" ~ '^[0-9]{2}/[0-9]{2}/[0-9]{4}$' and (   substring(c."authorized_at" from 4 for 2)::int not between 1 and 12   or substring(c."authorized_at" from 1 for 2)::int not between 1 and 31   or (substring(c."authorized_at" from 4 for 2)::int in (4, 6, 9, 11)       and substring(c."authorized_at" from 1 for 2)::int > 30)   or (substring(c."authorized_at" from 4 for 2)::int = 2       and substring(c."authorized_at" from 1 for 2)::int > 29)   or (substring(c."authorized_at" from 4 for 2)::int = 2       and substring(c."authorized_at" from 1 for 2)::int = 29       and not (substring(c."authorized_at" from 7 for 4)::int % 4 = 0                and (substring(c."authorized_at" from 7 for 4)::int % 100 <> 0                     or substring(c."authorized_at" from 7 for 4)::int % 400 = 0)))))) then 'DATE_IMPOSSIBLE'
            when c."authorized_at" ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}' and left(c."authorized_at", 10)::date > date '{{ var("as_of_date") }}' then 'DATE_FUTURE'
            when c."authorized_at" !~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}' and ( (c."authorized_at" ~ '^[0-9]{2}/[0-9]{2}/[0-9]{4}$'  and substring(c."authorized_at" from 4 for 2)::int between 1 and 12  and substring(c."authorized_at" from 1 for 2)::int between 1 and 31) or c."authorized_at" ~ '^[0-9]{4}\.[0-9]{2}\.[0-9]{2}$') then 'DATE_FORMAT_KNOWN'
            when c."authorized_at" ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}[ T][0-9]{2}:[0-9]{2}' and c."authorized_at" !~ '(Z|[+-][0-9]{2}:?[0-9]{2})$' then 'DATE_TZ_MISSING'
        end,
            'captured_at', case
            when btrim(c."captured_at") = '' or btrim(c."captured_at") in ('NULL', 'null', 'N/A', '-', '#N/D', '   ') then 'NULL_DISGUISED'
            when c."captured_at" !~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}' and ( c."captured_at" ~ '^[0-9]{2}/[0-9]{4}$' or (c."captured_at" ~ '^[0-9]{2}/[0-9]{2}/[0-9]{4}$' and (   substring(c."captured_at" from 4 for 2)::int not between 1 and 12   or substring(c."captured_at" from 1 for 2)::int not between 1 and 31   or (substring(c."captured_at" from 4 for 2)::int in (4, 6, 9, 11)       and substring(c."captured_at" from 1 for 2)::int > 30)   or (substring(c."captured_at" from 4 for 2)::int = 2       and substring(c."captured_at" from 1 for 2)::int > 29)   or (substring(c."captured_at" from 4 for 2)::int = 2       and substring(c."captured_at" from 1 for 2)::int = 29       and not (substring(c."captured_at" from 7 for 4)::int % 4 = 0                and (substring(c."captured_at" from 7 for 4)::int % 100 <> 0                     or substring(c."captured_at" from 7 for 4)::int % 400 = 0)))))) then 'DATE_IMPOSSIBLE'
            when c."captured_at" ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}' and left(c."captured_at", 10)::date > date '{{ var("as_of_date") }}' then 'DATE_FUTURE'
            when c."captured_at" !~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}' and ( (c."captured_at" ~ '^[0-9]{2}/[0-9]{2}/[0-9]{4}$'  and substring(c."captured_at" from 4 for 2)::int between 1 and 12  and substring(c."captured_at" from 1 for 2)::int between 1 and 31) or c."captured_at" ~ '^[0-9]{4}\.[0-9]{2}\.[0-9]{2}$') then 'DATE_FORMAT_KNOWN'
            when c."captured_at" ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}[ T][0-9]{2}:[0-9]{2}' and c."captured_at" !~ '(Z|[+-][0-9]{2}:?[0-9]{2})$' then 'DATE_TZ_MISSING'
        end,
            'created_at', case
    ...
    jsonb_build_object('id', c."id", 'payment_code', c."payment_code", 'order_id', c."order_id", 'payment_method_id', c."payment_method_id", 'status', c."status", 'amount', c."amount", 'currency', c."currency", 'installments', c."installments", 'authorized_at', c."authorized_at", 'captured_at', c."captured_at", 'created_at', c."created_at", 'updated_at', c."updated_at", 'deleted_at', c."deleted_at")              as original_payload
from captura c
