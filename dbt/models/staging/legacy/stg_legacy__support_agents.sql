...
select
    ...
    -- ── Valores tratados ─────────────────────────────────────────────────────
        ...
        case
            ...
            when c."agent_code" like '%;%' then c."agent_code"
        ...
        case
            ...
            when c."email" !~ '^[^@[:space:]]+@[^@[:space:]]+\.[^@[:space:]]+$' then c."email"
            when c."email" like '%;%' then c."email"
    ...
    jsonb_build_object('id', c."id", 'agent_code', c."agent_code", 'first_name', c."first_name", 'last_name', c."last_name", 'email', c."email", 'team', c."team", 'hired_at', c."hired_at", 'is_active', c."is_active", 'created_at', c."created_at", 'updated_at', c."updated_at", 'deleted_at', c."deleted_at")              as original_payload
from captura c
