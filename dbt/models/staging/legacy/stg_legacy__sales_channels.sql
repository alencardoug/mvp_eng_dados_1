...
-- Limpeza de `legacy.sales_channels` — camada `staging` (ADR-0016).
--
...
with captura as (
    ...
    select *
    from {{ source('legacy', 'sales_channels') }}
    where _airbyte_generation_id = coalesce(
        {{ legacy_snapshot_id() }}, (
        select max(_airbyte_generation_id) from {{ source('legacy', 'sales_channels') }}
    ))
...
select
    ...
    -- ── Valores tratados ─────────────────────────────────────────────────────
        ...
        case
            when btrim(c."channel_type") = '' or btrim(c."channel_type") in ('NULL', 'null', 'N/A', '-', '#N/D', '   ') then null
            when btrim(c."channel_type") not in ('web', 'app', 'store') then c."channel_type"
            else c."channel_type"
        end as "channel_type",
        case
    ...
    jsonb_strip_nulls(
        jsonb_build_object(
            ...
            'channel_type', case
            when btrim(c."channel_type") = '' or btrim(c."channel_type") in ('NULL', 'null', 'N/A', '-', '#N/D', '   ') then 'NULL_DISGUISED'
            when btrim(c."channel_type") not in ('web', 'app', 'store') then 'ENUM_UNKNOWN'
        end,
            'is_active', case
    ...
    jsonb_build_object('id', c."id", 'code', c."code", 'name', c."name", 'channel_type', c."channel_type", 'is_active', c."is_active", 'created_at', c."created_at", 'updated_at', c."updated_at", 'deleted_at', c."deleted_at")              as original_payload
from captura c
