...
-- Limpeza de `legacy.coupons` — camada `staging` (ADR-0016).
--
...
with captura as (
    ...
    select *
    from {{ source('legacy', 'coupons') }}
    where _airbyte_generation_id = coalesce(
        {{ legacy_snapshot_id() }}, (
        select max(_airbyte_generation_id) from {{ source('legacy', 'coupons') }}
    ))
...
select
    ...
    -- ── Valores tratados ─────────────────────────────────────────────────────
        ...
        case
            when btrim(c."discount_type") = '' or btrim(c."discount_type") in ('NULL', 'null', 'N/A', '-', '#N/D', '   ') then null
            when btrim(c."discount_type") not in ('percentage', 'fixed') then c."discount_type"
            else c."discount_type"
        end as "discount_type",
        case
            when btrim(c."discount_value") = '' or btrim(c."discount_value") in ('NULL', 'null', 'N/A', '-', '#N/D', '   ') then null
            when btrim(replace(replace(c."discount_value", 'R$', ''), ' ', '')) ~ '^-' then c."discount_value"
            when c."discount_value" !~ '^-?[0-9]+(\.[0-9]+)?$' then case when btrim(replace(replace(c."discount_value", 'R$', ''), ' ', '')) ~ ',[0-9]{1,2}$' then replace(replace(replace(replace(c."discount_value", 'R$', ''), ' ', ''), '.', ''), ',', '.') else replace(replace(replace(c."discount_value", 'R$', ''), ' ', ''), ',', '') end
            else c."discount_value"
        end as "discount_value",
        case
            when btrim(c."min_order_amount") = '' or btrim(c."min_order_amount") in ('NULL', 'null', 'N/A', '-', '#N/D', '   ') then null
            when btrim(replace(replace(c."min_order_amount", 'R$', ''), ' ', '')) ~ '^-' then c."min_order_amount"
            when c."min_order_amount" !~ '^-?[0-9]+(\.[0-9]+)?$' then case when btrim(replace(replace(c."min_order_amount", 'R$', ''), ' ', '')) ~ ',[0-9]{1,2}$' then replace(replace(replace(replace(c."min_order_amount", 'R$', ''), ' ', ''), '.', ''), ',', '.') else replace(replace(replace(c."min_order_amount", 'R$', ''), ' ', ''), ',', '') end
            else c."min_order_amount"
        end as "min_order_amount",
        case
            when btrim(c."max_redemptions") = '' or btrim(c."max_redemptions") in ('NULL', 'null', 'N/A', '-', '#N/D', '   ') then null
            else c."max_redemptions"
    ...
    jsonb_strip_nulls(
        jsonb_build_object(
            ...
            when c."code" like '%;%' then 'TEXT_DELIMITER'
            when c."code" ~ '(Ã.|Â.)' then 'TEXT_ENCODING'
            when c."code" <> btrim(c."code") or c."code" ~ '  ' then 'TEXT_WHITESPACE_CASE'
        end,
            'discount_type', case
            when btrim(c."discount_type") = '' or btrim(c."discount_type") in ('NULL', 'null', 'N/A', '-', '#N/D', '   ') then 'NULL_DISGUISED'
            when btrim(c."discount_type") not in ('percentage', 'fixed') then 'ENUM_UNKNOWN'
        end,
            'discount_value', case
            when btrim(c."discount_value") = '' or btrim(c."discount_value") in ('NULL', 'null', 'N/A', '-', '#N/D', '   ') then 'NULL_DISGUISED'
            when btrim(replace(replace(c."discount_value", 'R$', ''), ' ', '')) ~ '^-' then 'MONEY_NEGATIVE'
            when c."discount_value" !~ '^-?[0-9]+(\.[0-9]+)?$' then 'MONEY_LOCALE'
        end,
            'min_order_amount', case
            when btrim(c."min_order_amount") = '' or btrim(c."min_order_amount") in ('NULL', 'null', 'N/A', '-', '#N/D', '   ') then 'NULL_DISGUISED'
            when btrim(replace(replace(c."min_order_amount", 'R$', ''), ' ', '')) ~ '^-' then 'MONEY_NEGATIVE'
            when c."min_order_amount" !~ '^-?[0-9]+(\.[0-9]+)?$' then 'MONEY_LOCALE'
        end,
            'max_redemptions', case
            when btrim(c."max_redemptions") = '' or btrim(c."max_redemptions") in ('NULL', 'null', 'N/A', '-', '#N/D', '   ') then 'NULL_DISGUISED'
        end,
            'valid_from', case
    ...
    jsonb_build_object('id', c."id", 'code', c."code", 'campaign_id', c."campaign_id", 'discount_type', c."discount_type", 'discount_value', c."discount_value", 'min_order_amount', c."min_order_amount", 'max_redemptions', c."max_redemptions", 'valid_from', c."valid_from", 'valid_to', c."valid_to", 'is_active', c."is_active", 'created_at', c."created_at", 'updated_at', c."updated_at", 'deleted_at', c."deleted_at")              as original_payload
from captura c
