...
-- Limpeza de `legacy.order_items` — camada `staging` (ADR-0016).
--
...
with captura as (
    ...
    select *
    from {{ source('legacy', 'order_items') }}
    where _airbyte_generation_id = coalesce(
        {{ legacy_snapshot_id() }}, (
        select max(_airbyte_generation_id) from {{ source('legacy', 'order_items') }}
    ))
...
select
    ...
    jsonb_build_object('id', c."id", 'order_id', c."order_id", 'product_variant_id', c."product_variant_id", 'quantity', c."quantity", 'unit_price', c."unit_price", 'discount_amount', c."discount_amount", 'tax_amount', c."tax_amount", 'total_amount', c."total_amount", 'created_at', c."created_at", 'updated_at', c."updated_at", 'deleted_at', c."deleted_at")              as original_payload
from captura c
