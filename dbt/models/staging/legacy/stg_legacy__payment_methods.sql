...
-- Limpeza de `legacy.payment_methods` — camada `staging` (ADR-0016).
--
...
with captura as (
    ...
    select *
    from {{ source('legacy', 'payment_methods') }}
    where _airbyte_generation_id = coalesce(
        {{ legacy_snapshot_id() }}, (
        select max(_airbyte_generation_id) from {{ source('legacy', 'payment_methods') }}
    ))
...
select
    ...
    -- ── Valores tratados ─────────────────────────────────────────────────────
        ...
        case
            when btrim(c."method_type") = '' or btrim(c."method_type") in ('NULL', 'null', 'N/A', '-', '#N/D', '   ') then null
            when btrim(c."method_type") not in ('credit_card', 'debit_card', 'pix', 'boleto', 'wallet', 'voucher') then c."method_type"
            else c."method_type"
        end as "method_type",
        case
    ...
    jsonb_strip_nulls(
        jsonb_build_object(
            ...
            'method_type', case
            when btrim(c."method_type") = '' or btrim(c."method_type") in ('NULL', 'null', 'N/A', '-', '#N/D', '   ') then 'NULL_DISGUISED'
            when btrim(c."method_type") not in ('credit_card', 'debit_card', 'pix', 'boleto', 'wallet', 'voucher') then 'ENUM_UNKNOWN'
        end,
            'is_active', case
    ...
    jsonb_build_object('id', c."id", 'code', c."code", 'name', c."name", 'method_type', c."method_type", 'is_active', c."is_active", 'created_at', c."created_at", 'updated_at', c."updated_at", 'deleted_at', c."deleted_at")              as original_payload
from captura c
