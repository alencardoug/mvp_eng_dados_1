...
-- Limpeza de `legacy.warehouses` — camada `staging` (ADR-0016).
--
...
limpo as (
    ...
    select
        ...
        case
            ...
            when c."code" like '%;%' and (c."name" is null or btrim(c."name") = '') and (c."city" is null or btrim(c."city") = '') then c."code"
            when c."code" ~ '(Ã[-¿]|Â[-¿])' and c."code" !~ '(Ã|Â)[A-ZÁÂÃÉÊÍÓÔÕÚÇ ]' then convert_from(convert_to(c."code", 'LATIN1'), 'UTF8')
        ...
        case
            ...
            when c."name" like '%;%' and (c."city" is null or btrim(c."city") = '') and (c."state" is null or btrim(c."state") = '') then c."name"
            when c."name" ~ '(Ã[-¿]|Â[-¿])' and c."name" !~ '(Ã|Â)[A-ZÁÂÃÉÊÍÓÔÕÚÇ ]' then convert_from(convert_to(c."name", 'LATIN1'), 'UTF8')
        ...
        case
            ...
            when c."city" like '%;%' and (c."state" is null or btrim(c."state") = '') and (c."country" is null or btrim(c."country") = '') then c."city"
            when c."city" ~ '(Ã[-¿]|Â[-¿])' and c."city" !~ '(Ã|Â)[A-ZÁÂÃÉÊÍÓÔÕÚÇ ]' then convert_from(convert_to(c."city", 'LATIN1'), 'UTF8')
        ...
        case
            ...
            when c."state" like '%;%' and (c."country" is null or btrim(c."country") = '') and (c."capacity_units" is null or btrim(c."capacity_units") = '') then c."state"
            when c."state" ~ '(Ã[-¿]|Â[-¿])' and c."state" !~ '(Ã|Â)[A-ZÁÂÃÉÊÍÓÔÕÚÇ ]' then convert_from(convert_to(c."state", 'LATIN1'), 'UTF8')
        ...
        case
            ...
            when c."country" like '%;%' and (c."capacity_units" is null or btrim(c."capacity_units") = '') and (c."is_active" is null or btrim(c."is_active") = '') then c."country"
        ...
        case
            when btrim(c."capacity_units") = '' or btrim(c."capacity_units") in ('NULL', 'null', 'N/A', '-', '#N/D', '   ') then null
            when c."capacity_units" ~ '^-?[0-9]+$' and (c."capacity_units"::numeric < 0 or c."capacity_units"::numeric > 1000000) then c."capacity_units"
            when c."capacity_units" !~ '^-?[0-9]+$' and (  lower(btrim(c."capacity_units")) in ('um','dois','três','tres','quatro','cinco','seis','sete','oito','nove','dez')  or replace(btrim(c."capacity_units"), ',', '.') ~ '^-?[0-9]+\.0+$') then case lower(btrim(c."capacity_units")) when 'um' then '1' when 'dois' then '2' when 'três' then '3' when 'tres' then '3' when 'quatro' then '4' when 'cinco' then '5' when 'seis' then '6' when 'sete' then '7' when 'oito' then '8' when 'nove' then '9' when 'dez' then '10' else split_part(replace(btrim(c."capacity_units"), ',', '.'), '.', 1) end
            when c."capacity_units" !~ '^-?[0-9]+$' then c."capacity_units"
            else c."capacity_units"
...
select
    ...
    jsonb_strip_nulls(
        jsonb_build_object(
            ...
            'code', case
            when btrim(c."code") = '' or btrim(c."code") in ('NULL', 'null', 'N/A', '-', '#N/D', '   ') then 'NULL_DISGUISED'
            when c."code" like '%;%' and (c."name" is null or btrim(c."name") = '') and (c."city" is null or btrim(c."city") = '') then 'TEXT_DELIMITER'
            when c."code" ~ '(Ã[-¿]|Â[-¿])' and c."code" !~ '(Ã|Â)[A-ZÁÂÃÉÊÍÓÔÕÚÇ ]' then 'TEXT_ENCODING'
            when c."code" <> btrim(c."code") or c."code" ~ '  ' then 'TEXT_WHITESPACE_CASE'
        end,
            'name', case
            when btrim(c."name") = '' or btrim(c."name") in ('NULL', 'null', 'N/A', '-', '#N/D', '   ') then 'NULL_DISGUISED'
            when length(c."name") = 24 then 'TEXT_TRUNCATED'
            when c."name" like '%;%' and (c."city" is null or btrim(c."city") = '') and (c."state" is null or btrim(c."state") = '') then 'TEXT_DELIMITER'
            when c."name" ~ '(Ã[-¿]|Â[-¿])' and c."name" !~ '(Ã|Â)[A-ZÁÂÃÉÊÍÓÔÕÚÇ ]' then 'TEXT_ENCODING'
            ...
            when c."city" like '%;%' and (c."state" is null or btrim(c."state") = '') and (c."country" is null or btrim(c."country") = '') then 'TEXT_DELIMITER'
            when c."city" ~ '(Ã[-¿]|Â[-¿])' and c."city" !~ '(Ã|Â)[A-ZÁÂÃÉÊÍÓÔÕÚÇ ]' then 'TEXT_ENCODING'
            ...
            when c."state" like '%;%' and (c."country" is null or btrim(c."country") = '') and (c."capacity_units" is null or btrim(c."capacity_units") = '') then 'TEXT_DELIMITER'
            when c."state" ~ '(Ã[-¿]|Â[-¿])' and c."state" !~ '(Ã|Â)[A-ZÁÂÃÉÊÍÓÔÕÚÇ ]' then 'TEXT_ENCODING'
            ...
            when c."country" like '%;%' and (c."capacity_units" is null or btrim(c."capacity_units") = '') and (c."is_active" is null or btrim(c."is_active") = '') then 'TEXT_DELIMITER'
            ...
            'capacity_units', case
            when btrim(c."capacity_units") = '' or btrim(c."capacity_units") in ('NULL', 'null', 'N/A', '-', '#N/D', '   ') then 'NULL_DISGUISED'
            when c."capacity_units" ~ '^-?[0-9]+$' and (c."capacity_units"::numeric < 0 or c."capacity_units"::numeric > 1000000) then 'NUM_OUT_OF_RANGE'
            when c."capacity_units" !~ '^-?[0-9]+$' and (  lower(btrim(c."capacity_units")) in ('um','dois','três','tres','quatro','cinco','seis','sete','oito','nove','dez')  or replace(btrim(c."capacity_units"), ',', '.') ~ '^-?[0-9]+\.0+$') then 'NUM_TEXT_EQUIV'
            when c."capacity_units" !~ '^-?[0-9]+$' then 'NUM_AMBIGUOUS'
        end,
            'is_active', case
    ...
    jsonb_build_object('id', c."id", 'code', c."code", 'name', c."name", 'city', c."city", 'state', c."state", 'country', c."country", 'capacity_units', c."capacity_units", 'is_active', c."is_active", 'created_at', c."created_at", 'updated_at', c."updated_at", 'deleted_at', c."deleted_at")              as original_payload
from captura c
