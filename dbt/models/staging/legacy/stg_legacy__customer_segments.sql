...
-- Limpeza de `legacy.customer_segments` — camada `staging` (ADR-0016).
--
...
with captura as (
    ...
    select *
    from {{ source('legacy', 'customer_segments') }}
    where _airbyte_generation_id = coalesce(
        {{ legacy_snapshot_id() }}, (
        select max(_airbyte_generation_id) from {{ source('legacy', 'customer_segments') }}
    ))
...
select
    ...
    jsonb_build_object('id', c."id", 'code', c."code", 'name', c."name", 'description', c."description", 'is_active', c."is_active", 'created_at', c."created_at", 'updated_at', c."updated_at", 'deleted_at', c."deleted_at")              as original_payload
from captura c
